...
class User(Base):
    ...
    blogs = relationship("Index", back_populates="creator")
```
...
def get_all(db: Session):
    blogs = db.query(models.Index).all()
    return blogs

def create(request: schemas.Index, db: Session):
    new_blog = models.Index(title=request.title, body=request.body, user_id=1)
    db.add(new_blog)
    db.commit()
    db.refresh(new_blog)
    return new_blog
...
def destroy(id: int, db: Session):
    blog_to_delete = db.query(models.Index).filter(models.Index.id == id)

    if not blog_to_delete.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Index with id {id} not found.")
    blog_to_delete.delete(synchronize_session=False)
    db.commit()
    return {'done'}

def update(id: int, request: schemas.Index, db: Session):
    blog = db.query(models.Index).filter(models.Index.id == id)
    if not blog.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Index with id {id} not found")
...
def show(id: int, db: Session):
    blog = db.query(models.Index).filter(models.Index.id == id).first()
    if blog:
        return blog
    else:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Index with the id {id} is not available")
```

### Core :

- In this folder we gonna Create 3 files `Auth.py` and `Index.py` and `User.py`, all of this files are the routes for our API.
...
- Now we need to add the `Index.py`, relate to the API where we provide all Requirements.
- `Index.py`:
...
def create(request: schemas.Index, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
...
def update_blog(id: int, request: schemas.Index, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
...
app = FastAPI(
    title="SecureAPI",
    description="Hello thank you for reading my Index UwU",
    version="1.0.0",)

...
![Peer-to-Peer Model](./peer-to-peer.png)
...
![Publish-Subscribe Model](./publish-subscribe.webp)
...
The Pub-Sub system operates as follows: -**Message Publication**: A publisher sends a message to the message broker, specifying the topic or channel to which the message belongs. -**Message Routing**: The message broker routes the message to all subscribers that have subscribed to the topic or channel. -**Message Delivery**: The message broker delivers the message to the subscribers, who can then process the message as needed.
